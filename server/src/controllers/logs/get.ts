...
const ALLOWED_FIELDS_PER_MODEL = {
...
const handler = async (
  req: Request<{}, {}, LogsAPITypes.GetRequestBody>,
  res: Response<LogsAPITypes.GetResponseData>,
  _next: NextFunction,
) => {
  ...
  try {
    ...
    if (error instanceof Error) {
      LoggingService.log({
        ...
        level: "error",
        traceId: req.traceId,
        message: "Unexpected error during log fetching",
        details: {
          error: error.message,
          stack: error.stack,
          logIds: req.body.logIds?.join(", "),
        },
        _references: {
          logIds: "Log",
        },
